...
%   $Revision: 1.23 $  $Date: 2001/03/27 19:55:18 $
...
numberOfVariables = n;
% get options out
active_tol = optimgetv61(options,'ActiveConstrTol',sqrt(eps)); % leave old optimget
pcmtx = optimgetv61(options,'Preconditioner','aprecon') ; % leave old optimget

gradflag =  strcmp(optimget(options,'Jacobian',defaultopt,'fast'),'on');
typx = optimget(options,'TypicalX',defaultopt,'fast') ;
mtxmpy = optimgetv61(options,'JacobMult',defaultopt,'fast') ;
if isempty(mtxmpy)
    mtxmpy = @atamult;
end
pcflags = optimgetv61(options,'PrecondBandWidth',defaultopt,'fast') ;
tol2 = optimgetv61(options,'TolX',defaultopt,'fast') ;
tol1 = optimgetv61(options,'TolFun',defaultopt,'fast') ;
tol = tol1;
itb = optimgetv61(options,'MaxIter',defaultopt,'fast') ;
maxfunevals = optimgetv61(options,'MaxFunEvals',defaultopt,'fast') ;
pcgtol = optimgetv61(options,'TolPCG',defaultopt,'fast') ;  % pcgtol = .1;
kmax = optimgetv61(options,'MaxPCGIter',defaultopt,'fast') ;
if ischar(typx)
...
showstat = optimgetv61(options,'showstatus','off'); % no default, so leave with slow optimgetv61
switch showstat
case 'iter'
   showstat = 2;
case {'none','off'}
...
%   Evaluate F and J
   if ~gradflag % use sparse finite differencing
      ...
      [A,findiffevals] = sfdnls_mod(xcurr,fvec,Jstr,group,[],funfcn{3},YDATA,varargin{:});
...
while ~ex 
   ...
   if ~ex 
      ...
      if ~gradflag %use sparse finite-differencing
         ...
         [newA, findiffevals] = sfdnls_mod(xcurr,newfvec,Jstr,group,[],funfcn{3},YDATA,varargin{:});
