function[out, param] = load_shot(shot, nBDmodes, timeBound, line, frame_sum, plots, s)
...
if param.calcError
    ...
    [tempU, velU, dummy, dummy, dummy] = calcPhysics(fit_par + dPar, param);

    % Caulculate Lower Error Physics

    [tempL, velL, dummy, dummy, dummy] = calcPhysics(fit_par - dPar, param);
