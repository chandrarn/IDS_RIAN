...
% PHASE 1 -----------------------------------------------------------------
doPHASE1 = 1;
shot1 = 14062506; % always use this one
shot2 = 14062507; % set 'shot2' to zero if only using one fiber
xlim = []; % pixels for initial plotting, leave empty if unsure
ylim = [25 35]; % pixels for initial plotting, leave empty if unsure
...
firstCenter = [33, 31]; % Center position of first channel, [real, wavelength]
lastCenter = [339, 31]; % Center position of last channel, [real, wavelength]
...
LAMBDA = 1e-9 * [465.025]; % [m], plasma lines in descending wavelength
VOLTAGE = 900; % image intensifier voltage, '0' if not using
MASS = [12]; % [AMU], corresponding to LAMBDA
