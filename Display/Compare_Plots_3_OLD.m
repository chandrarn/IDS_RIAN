...
%close all; 
%clc;
cd('T:\IDS\Data Repository\TEMP'); % where corrected IDS data is kept My data
...
tLimMS = [.72 2.12]; % time points in ms

interval = 6.9; % cine frame interval, in uS
...
timeLim = [.72 2.12]%[1.0 1.9];
%timeLim = [1.3,2.1];
%timeLim= [];
timePoint = []; % time point limits
...
if timeInMs
    dat.time=dat.time;
    x = dat.time(1:end);%*1e-3;% TEMP MULTIPLY, SOME SHOTS IN S NOT mS
    xlab = {'Time [ms]'};
else 
    x = 1:length(dat.time);
