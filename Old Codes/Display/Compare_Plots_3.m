...
%cd('/media/alfventemp/IDS/Data Repository/')
%% Plot Settings
...
shiftVel = 10; % km/s velocty shift
                ...
                % -7 for 129499, etc.
torPlot = 0; % true for toroidal array (fibers 1 to 36)
...
screenInt = 1; % run velocity and temperature through a filter to discard points with low intensity
scInt = 50; % lower limit for Area
...
jLim = [-20 30]; % Limit for current plots
...
cd('T:\IDS\Display');
%cd('/media/alfventemp/IDS/Display')
...
if ~square
    ...
    ha = plot(dat.ItorTime, dat.Itor, plt1);
    ...
    %plot injectors, 2/3
    if ~hitsi3 
        hb = plot(dat.iinjxTime, dat.iinjx, plt2);
        hc = plot(dat.iinjyTime, dat.iinjy, plt3);
        ...
        hb = plot(dat.iinjaTime,  dat.iinja, plt2);
        hc = plot(dat.iinjbTime,dat.iinjb, plt3);
        hd = plot(dat.iinjcTime, dat.iinjc, plt4);
        set(hb, 'LineWidth', lnwdth);
        set(hc, 'LineWidth', lnwdth);
...
%     set(gca, 'YLim', jLim, 'YTick', [-100 -50 0 50 100]);
    set(gca, 'YLim', jLim, 'YTick', [-40 -20 0 20 40]);
