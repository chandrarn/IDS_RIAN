...
for i = 1:2
%     if i ==1
%         load(['Ndat' num2str(prefix(i)*1000000 + shot) '10.mat'])
%     elseif i == 3 % special nimrod temperature case
%         load(['dat' num2str(prefix(i)*1000000 + shot) '.mat'])
%         %dat.time = dat.time(1:end-1);
%     elseif i == 2 % special psitet velocity case
%         load(['dat' num2str(prefix(i)*1000000 + shot) '.mat'])
%         dat.time = dat.time(1:end);
%     end
    if i ==1
        %load(['dat' num2str(prefix(i)*1000000 + shot) '10.mat'])
        load(['dat' num2str(shots(i)) '10.mat'])
%         dat.time = dat.time *1e3;
    ...
    %load(['Phase' num2str(shots(i)) '.mat']);
    if(i==1)
        %PhaseVelocity.Velocity = PhaseVelocity.Velocity +10;
...
% title(['Comparison to Simulation, shot: ' num2str(shots(1))]);
