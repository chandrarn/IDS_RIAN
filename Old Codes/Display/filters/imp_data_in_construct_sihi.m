function [cb_field, ins_depth] = imp_data_in_construct_sihi(array, ...
...
if tin(1) < ts_dum || tin(end) > te_dum
    you_have_a_time_problem;
end
...
for j = 1: N
    ...
    elseif isa(tminr, 'numeric')
        ...
        bw_rad = mdsvalue(['sihi_smooth(sub_baseline_string("\' rnode '"))']);
        if filterdata == 2;
            % bandpass filter
...
%         cbw_rad = cbw_rad - b_offset_rad;
    ...
    elseif isa(tminp, 'numeric')
        ...
        bw_pol = mdsvalue(['sihi_smooth(sub_baseline_string("\' pnode '"))']);
        if filterdata == 2;
            % bandpass filter
...
%         cbw_pol = cbw_pol - b_offset_pol;
    ...
    elseif isa(tmint, 'numeric')
        ...
        bw_tor = mdsvalue(['sihi_smooth(sub_baseline_string("\' tnode '"))']);
        if filterdata == 2;
            % bandpass filter
