function [cb_field, ins_depth] = imp_data_in_construct_rms(array, ...
...
for j = 1: N
    ...
    elseif isa(tminr, 'numeric')
        ...
        bw_rad = mdsvalue(['sqrt(sihi_smooth(sigadd(sub_baseline_string("\' ...
            rnode '"), -sihi_smooth(sub_baseline_string("\' rnode '")))^2))']);
        if filterdata == 2;
            % bandpass filter
    ...
    elseif isa(tminp, 'numeric')
        ...
        bw_pol = mdsvalue(['sqrt(sihi_smooth(sigadd(sub_baseline_string("\' ...
            pnode '"), -sihi_smooth(sub_baseline_string("\' pnode '")))^2))']);
        if filterdata == 2;
            % bandpass filter
    ...
    elseif isa(tmint, 'numeric')
        ...
        bw_tor = mdsvalue(['sqrt(sihi_smooth(sigadd(sub_baseline_string("\' ...
            tnode '"), -sihi_smooth(sub_baseline_string("\' tnode '")))^2))']);
        if filterdata == 2;
            % bandpass filter
