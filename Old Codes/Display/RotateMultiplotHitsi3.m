...
switch plotType
    case 1
        %Ndata = NIDS.vel+6;
        Idata = Idat.vel(:,chanrange);% add offset if neccessary
        titles = 'Velocity';
...
for j = 1:size(Idata,1) % how many time points
...
%size(dat.time);
%size(data');
t(1,:) = plot(ax,Idat.impacts(chanrange),Idata,'color',colors(2,:),'LineWidth',1.5);
...
for i = 1:size(Idata,1)
    ...
    text(Idat.impacts(chanrange(1))+0,y,num2str(Idat.time(i),4),'fontsize',14);
...
legend(ax,'Location','BestOutside','999 IDS');
