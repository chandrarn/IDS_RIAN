...
shot = [126690];
spSum = 1; % look at chord sum
tSum = 0; % look at time pattern

saveFigure = 0; % save figure to file
fileName = 'S:\HIT_SI\APS\2012\Poster\IDS_chSum_126690'; % file name for .png image
...
if length(shot) > 1
    title(['Spatially Averaged ' t1 ' ' t2 ', Shots ' num2str(shot(1)) ' and ' num2str(shot(2))]);
else
...
ylabel('Current [kA]');
if length(shot) > 1
    legend('I_{INJ} X','I_{INJ} Y',['I_{TOR} ' num2str(shot(1))],['I_{TOR} ' num2str(shot(2))],...
