...
function [ Phase,Velocity,Impacts ]= VelocityPhase(dat,TimeBounds,samples,Is14,shift)
 ...
 close all;
        ...
        if(Is14) % if 14.5, set end time bound to the velocity time at
            ...
            % INJ CYCLE
            TimeBounds(2)=dat.time(dummy(1)+10*samples-1)
        ...
        VelPoints=cast(linspace(VelminBound,VelmaxBound,NumVelPoints),'uint8')'
        ...
        for(point=VelPoints')%VelPoints(1):VelPoints(end)
                ...
                % only of size 500.
                x=linspace(dat.iinjxTime(minBound),dat.iinjxTime(maxBound),500)';
                %%
                %%
...
%                 f=1/avdif
        ...
        %point
        if(Is14)
            PhaseFinal=zeros(SamplesPerInjectorCycle,samples);%take the median of the three phase values/
            ...
            for(i=1:SamplesPerInjectorCycle)
                ...
                figure;
                errorbar(linspace(-23,28,size(dat.vel,2)),MEAN',STD);
                ...
                plot(linspace(-23,28,size(dat.vel,2)),TempVel(:,1),linspace(-23,28,size(dat.vel,2)),TempVel(:,2),linspace(-23,28,size(dat.vel,2)),TempVel(:,3),linspace(-23,28,size(dat.vel,2)),TempVel(:,4),linspace(-23,28,size(dat.vel,2)),TempVel(:,5),linspace(-23,28,size(dat.vel,2)),median(TempVel,2)','-o');
                legend('First','Second','Third','Fourth','Fifth','Median');
                title(strcat(title1, ' 5 time points'),'FontSize',15);
