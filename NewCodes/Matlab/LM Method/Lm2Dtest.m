...
function [p_fit,Chi_sq,sigma_p,sigma_y,corr,R2,cvg_hst] = Lm2DTest%(fit_par, x, z)
close all; 
clear all;
...
dp = [0.0002, 0.0002, 0.0002, 0.0002, 0.0002, 0.0002];
...
[sx sy]=size(X);
 ...
 p_true = [1242.04667365151,97.8073597485189,40.2152933825534,1.50572796181989,2.14437493106966,28.2643984590859];
 ...
 t = [x(:,1) x(:,2) ];
p_min =  [1000.04667365151,90.8073597485189,30.2152933825534,1.00572796181989,1.54437493106966,20.2643984590859]; % minimum expected parameter values
p_max = [1700.04667365151,105.8073597485189,50.2152933825534,2.0572796181989,2.74437493106966,35.2643984590859]; % maximum expected parameter values
p_init = [1000.04667365151,97.8073597485189,40.2152933825534,2.10572796181989,3.64437493106966,20.2643984590859]; % initial guess for parameter values
z_dat= x(:,3);
...
lm(@singletGauss2DLM, p_true, t, z_dat, .001,dp, p_min,p_max,0)
%lm(func, p, t, y_dat, weight, dp, p_min, p_max, c)

Z=singletGauss2DLM(t,p_fit,0);
plot3(X,Y,reshape(Z,sx,sy),'*');
