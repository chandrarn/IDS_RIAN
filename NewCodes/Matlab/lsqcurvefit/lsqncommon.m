...
try
    ...
    addpath('C:\Users\HITSI\Documents\GitHub\IDS_RIAN\NewCodes\Matlab\lsqcurvefit');
catch
...
mtxmpy = optimgetv61(options,'JacobMult',[]); % use old
...
if isequal(OUTPUT.algorithm,large)
    if ~gradflag % provide sparsity of Jacobian if not provided.
        Jstr = optimgetv61(options,'JacobPattern',[]);
        if isempty(Jstr)  
