function[PEAKS, REL_INT, par, fits] = calGauss2D(peaks, data, brightWing, xWing, indivFit)
...
%     disp(['xBound = ' num2str(xBound)]);
    ...
    if plotFits
        clf; % Plot Raw Data ----------------------------
        ax(1) = axes('Parent', h1, 'Position', [0.1 0.1 0.2 0.8], 'FontSize', fntsz);
        h3 = surf(X, Y, Z);
        hold on;
        shading interp;
        colormap jet;
...
%     guess = [vol, Zmaxx,Zmaxy , sigx, sigy, min(Z(:))];
    lb = [0.3*vol, guess(2)-0.5, guess(3)-1, 0.7*guess(4), 0.7*guess(5), 0]; % lower bound
    ub = [4.8*vol, guess(2)+0.5, guess(3)+1, 2.3*guess(4), 2.3*guess(5), 0.5*max(Z(:))]; % upper bound
