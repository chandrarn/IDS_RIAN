...
% PHASE 1 -----------------------------------------------------------------
doPHASE1 = 1;
shot1 = 160428001; % always use this one
shot2 = 160428002; % set 'shot2' to zero if only using one fiber
xlim = []; % pixels for initial plotting, leave empty if unsure
ylim = []; % pixels for initial plotting, leave empty if unsure

% PHASE 2 -----------------------------------------------------------------
doPHASE2 = 1;
% chanNums = [1:4, 6:21, 23:72]; % 5 and 22 are dead
chanNums = [1:62]; % 5 dead, 22 weak but including - 5/31/13
firstCenter = [73, 56]; % Center position of first channel, [real, wavelength]
lastCenter = [329, 55]; % Center position of last channel, [real, wavelength]
...
doPHASE5 = 0;
shot3 = 150623004; % Motor Calibration Shot
shot4 = 150623005; % Optional second motor calibration shot
...
% PHASE 5 C? --------------------------------------------------------------
doPHASE5C = 0;
motorCalShot = 150623006;
binChanMotor = 273;178;
channel = 55;
...
saveToShots = [150505022:150505023,150506009:150506011];
impactsFile = 'T:\RChandra\NewCodes\Geometry\impacts2.mat';
...
LAMBDA = 1e-9 * [464.913, 464.742, 464.308, 464.181]; % [m], plasma lines in descending wavelength
VOLTAGE = 0; % image intensifier voltage, '0' if not using
MASS = [16, 12, 14,16]; % [AMU], corresponding to LAMBDA
