...
% in(1).shot = 160728012;
% in(1).line=2;
% in(1).color = {'g';'g'};%[66, 188, 244]./255};
% in(1).style = {'-','--'};
% in(1).legend = [num2str(in(1).shot) ': -52.3kA'];
...
% in(1).fftPlot = [1]; % FFT of signal, n frequencies
% in(1).AnalysisTitle=[ 'HIT-SI3: 0-120-240 Phasing, Gain 2.9, ' lines{in(1).line}];
% in(1).phaseShift = 2*pi -pi/2 -pi;% The +pi is to overlay with the positive shot
% in(2)=in(1);
% in(2).phaseShift = 2*pi -pi/2;
% in(2).shot = 160728013;
% in(2).color = {'r';'r'};%[182, 244, 66]./255};
% in(2).legend = [num2str(in(2).shot) ': +65.6kA'];
...
in(1).fftPlot = [1]; % FFT of signal, n frequencies
in(1).AnalysisTitle='HIT-SI: 0-90 Phasing, CIII'; % Note: Line doesnt change by itself
in(1).phaseShift =-pi/2;
% 129450, 129451
% in(2) = in(1);
% in(2).shot=129450;
...
flipLoImpact=1.*(in(1).shot>229499); % correct for lower fiber being rotated 180 about impact 35 
...
for n = 1:length(in)
    ...
    if ~(exist('dat','var') && strcmp(dat(1).title,['Shot ' num2str(in(n).shot)])) % speed up runtime if data is already loaded
        ...
        dat = trimRange(input.dat, chan_range, plotError,timebound.*(1./in(n).timeScale),[]); % for some reason, this wont save to workspace
                ...
                if in(n).fftPlot
                    ...
                    elseif Analysis==2
                        ...
                        if ~plotError
                            ...
                            error = nanmean(dat(in(n).line).velU(:,doubleplot(i,:)))./(2*pi*14500) .*1e5; % convert error to centemeters displacement
                            if any(dispSupress(n,:))
                                dataDispl(dispSupress(n,:),i)=NaN;
                            end
...
%                           'MarkerEdgeColor',[in(n).color{1}]);
                ...
                if in(n).shot == 160728013 && in(n).line==2 && plotType ==1
                    dataPhase(3:12,1) = dataPhase(3:12,1)-2*pi;
                    dataPhase(:,1) = dataPhase(:,1)+2*pi;
                end
...
%                         end
                    ...
                    if includeTemp
                        if any(tempSupress(n,:,i))
                            dat(in(n).line).temp(:,tempSupress(n,:,i))=NaN;
                        end
                        if ~plotError
