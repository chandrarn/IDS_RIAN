...
in(1).AnalysisTitle='NIMROD: 0-120-240 Phasing, OII';
%% 160728
% in(2)=in(1);
% in(2).shot = 160728012;
% in(2).line=1;
...
%                           'MarkerEdgeColor',[in(n).color{1}]);
                         t3(n)=plot(ax7,dat(1).impacts(1:size(data,2)),-(dataAvg(:,1)-dataAvg(:,2)),'color',[in(n).color{1}],'marker','*','LineWidth', lnwdth, 'LineStyle', in(n).style{1},...
                         'MarkerEdgeColor',[in(n).color{1}]);
...
%                     'MarkerEdgeColor',[in(n).color{1}]);
            ...
            if Analysis==1 & in(n).doubleplot==1
                plot(dat(1).impacts(1:size(data,2)),-(dataAvg(:,1)-dataAvg(:,2)),'color' ,'k','marker','*','LineWidth', lnwdth, 'LineStyle', in(n).style{1});
                plot(xlim,[0,0],'--k')
