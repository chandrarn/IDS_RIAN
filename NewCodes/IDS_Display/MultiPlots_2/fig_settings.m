...
function [h,ax] = fig_settings(plt,Analysis,in)
fntsz = 19;
lnwdth = 1.5;
...
elseif Analysis==2 && ~isempty(in(1).fftPlot)
    ...
    ax(6) = axes('Parent',h(6),'Position',[0.15,.08,.8,.25]); hold on; grid on;box on;
    ax(7) = axes('Parent',h(6),'Position',[0.15,.38,.8,.25]); hold on; grid on;box on;
    ax(8) = axes('Parent',h(6),'Position',[0.15,.68,.8,.25]); hold on; grid on;box on;
