...
shots = [151217016:151217026];
nBDmodes = [10]; % number of modes to save after BD filtering. Leave blank for no BD filtering
timeBound = [1:250]; % time point bounds for BD filtering.  Leave blank to use whole movie
fiberBound = [138, 171]; % manually define realspace pix of dead zone between fibers
...
%     NATIVEvalue(Data.getData().data);
    ...
    [out, param] = load_shot(shots(n), nBDmodes, timeBound, line, frame_sum, plots, s,useTree,auto,fiberBound);
    t = toc;
