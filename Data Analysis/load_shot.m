function[out, param] = load_shot(shot, nBDmodes, timeBound, line, frame_sum, plots, s,useTree,auto,fiberBound)
...
[param, options] = loadParams(shot, line, useTree, s);

assignin('base','param',param); %Throw to workspace
...
if isempty(nBDmodes)
    ...
    [data, time] = BDfilter(shot, nBDmodes, timeBound, param, s, auto, fiberBound);%[36 37]    
end
...
[temp, vel, int, param] = calcPhysics(fit_par, param, s,line);

%% Error
if param.calcError
    % Caulculate Upper Error Physics

    [tempU, velU, dummy, dummy] = calcPhysics(fit_par + dPar, param, s,line);

    % Caulculate Lower Error Physics

    [tempL, velL, dummy, dummy] = calcPhysics(fit_par - dPar, param, s,line);
...
assignin('base','OrigVel',vel);
assignin('base','OrigTemp',temp);
